...
;; - figure out if algorithm has reached goal
;; - number of finsts: control



;; - find shortest solution to the maze  (possibly just simplify using
;; backtrack algorithm)
;; - also note the solution time it predicts
...
(defparameter *robot-width* 24)  ; 0.60m/0.025
...
(load "/usr/local/asdf-install/site/cl-vectors-0.1.3/paths-package.lisp")
...
(defparameter *background-image-file* "/Users/dr/MURI/path/building.gif")
...
(defun transp (x) x)
(defvar *canvas-height* nil)
(defvar *canvas-width* nil)
(defun flip-point (p)
...
;; 			      (format nil "~C~C" (+ 65 x) (+ 65 y)  ))
	...
	(setq start-pos (car some-challenge) 
      ...
      (vecto::rectangle  (* cell-width (+ 0.0 (car start-pos))) (transp (* cell-height (+ (second start-pos) 0.0)))
			 (* 1.0 cell-width)  (* 1.0 cell-height))
      (vecto::rectangle  (* cell-width (+ 0.0 (car end-pos))) (transp (* cell-height (+ (second end-pos) 0.0)))
			 (* 1.0 cell-width)  (* 1.0 cell-height))
...
(defun make-maze (&optional cells-h cells-v)
  "Makes a maze.
CELLS-H specifies width, CELLS-V specifies height.
Returns a list of the form (start-pos end-pos top-nodes).
The two -pos elements are lists of the form (x y node).
...
The resulting graph is circular."
  ...
  (let* ((cell-width 20) (cell-height 20)
	 (*canvas-width* (* cell-width (+ 2 cells-h)))
	 (*canvas-height* (* cell-height (+ 2 cells-v)))
	 (wall-ratio 5)	;; out of 10
	 (v-wall-ratio 5)
	 (start-end-top nil)
	 )

    (vecto::with-canvas (:width *canvas-width* :height *canvas-height*)
      (setq start-end-top (draw-new-maze *square-width* cells-h cells-v wall-ratio v-wall-ratio))

      (when *graphical-solve*
      (setq *solution-found*
	    (graphical-solve *temp-file-gif*
...
(defun perpendicular-norm (from to &optional length)
  ...
  (if (equal from to)
	...
	(vectors::p* p (/ (or length 1) (max 1 (vectors::point-norm p)))))))
...
(defun graphical-solve (image-file start end &optional maxpathlen)
...
;  (walk-to (cons 300 300))
  ...
  (let* (
    ...
    (loop for step from 0 to (expt (/ canvas-width *square-width*) 1) ;; heuristic
       until (or (equal (point-square end) (point-square *visual-attention-loc*))
		 (and maxpathlen (> (waypoints-len *visual-trace-path*) maxpathlen))
		 (< (vectors::point-distance end *visual-attention-loc*) *square-width*))
       do
	 (print (vectors::point-distance end *visual-attention-loc*))
	 (move-to-next-target image end))
    ;; did we reach the goal?
    (or (equal (point-square end) (point-square *visual-attention-loc*))
	(< (vectors::point-distance end *visual-attention-loc*) *square-width*))
    ))
	   ...
	   (loop for xd  in '(0) for yd in '(0) ;in (list 0.5 -0.5 0.5 -0.5) for yd in (list -0.5 0.5 0.5 -0.5) 
		...
		(loop for angle from (+ angle (* -2 pi)) to (+ angle (* 2 pi)) by (/ pi 20) do
			 ...
			 (let* ((target-point (longest-unobstructed-path-segment 
				...
				(length-scaled (sample-path-density image target-point end)) ;;(+ (* 1 goal-dist) (* 0 len)))
				)
			   ;; (if target-point
			   ;; 			    (mark-point target-point))
			   (when (and target-point
				      goal-dist
				      (> len 2) ; do not stay in the same place
				      ...
				      (or (> len  *square-width*) ;; force movements [not mentioned in paper - can be left out]
...
(defparameter *blue-density* 0.1 "relative density of blue from 0 to 1")
	  ...
	  (let ((point (vectors::p+ start (vectors::p* dir i))))
	    ...
	    (loop for j from -40 to +40 by 4 sum
		 (let* ((point2 (vectors::p+ start (vectors::p* spp j)))
			(col (if (and (< (1+ (car point2)) canvas-width) (< (1+ (cdr point2)) canvas-height) (> (car point2) 0) (> (cdr point2) 1))
...
(defparameter *always-check-left-and-right* t)
(defun walk-to (point &optional backtracking skippy-image end)
  ...
  (let* ((pts nil)
	 ...
	 (space-samples))
    ;; collect points
    (when (and *visual-attention-loc* (not (equal *visual-attention-loc* point)))
      (loop for i from 0 to 1 by (max 0.05 (/ *square-width* 2 (max 1 (vectors::point-distance *visual-attention-loc* point))))
	 ...
	 (let* ((delta (vectors::p* dir i))
	   ...
	   (when (or *always-check-left-and-right* (and backtracking (> (vectors::point-norm delta) *square-width*)))  ;; are we going towards the goal?
...
(defun visualize-path (points &optional smooth color)
...
;; 2 - white
  ...
  (let ((wall (list 0))
	...
	(canvas-height (skippy::height skippy-image))
	(canvas-width (skippy::width skippy-image)))
    (and (< (1+ x) canvas-width) (< (1+ y) canvas-height) (> x 0) (> y 0)
    (if fast
      ...
      (loop with iter =
	   ...
	   (when prev-knot
	     ...
	     (let* ((len (vectors::point-distance knot prev-knot))
	       ...
	       (loop for i from 0 to len by (if fast 3 0.5)
		  for x = (+ startx (* xt i)) 
...
(defun init-vis ()
  (setq *visual-attention-loc* nil *visual-trace-path* nil *visual-finsts* nil))
  ...
  (let ((solved nil)
    ...
    (loop for trial from 1 to n do
	 ...
	 (let ((*temp-file*
	   ...
	   (let* ((mm (make-maze *maze-size* *maze-size*))
      ...
      (loop for challenge in (fifth start-end-top-imagefile) do
	   ...
	   (let* (;; (canvas-width (skippy::width *image*)) ; prepare vecto image with the "track"
	     ...
	     (find-waypoints-draw-to-current-canvas 
	      (third (car challenge)) (third (cdr challenge)) 
...
(defun find-waypoints-draw-to-current-canvas (start-node end-node image)
  ...
  (let ((*image* image))
    ...
    (let* ((*verbose* t))
      ...
      (plan (graph-name start-node) (graph-name end-node) 18.0)
      )
    ;; callback to visual-search
    (print (format nil "walk: ~s  visual: ~s" *walk-count* *visual-count*))
    (when (and *display-results*  (> *walk-count* (+ 1 *visual-count* ))) ; if more than one memory step
      (visualize-path *visual-trace-path*)
...
(defun visual-search (current-chunk target-chunk)
  (incf *visual-count*)
  (let ((cur-loc (name-location current-chunk))
...
(defun read-geo-data (path id time)
  "returns nil if file doens't exist"
	 (let ((fp (open (format nil "~a~a_~a.tfw" path id time) :direction :input :if-does-not-exist nil))
...
(defvar *write-png* nil)
(defun run-over-geo-files (&optional blue-density robot-width straight-line always-check repetitions file)
  
  (let* ((start-at-time nil) (model-active t)
	 (tme (get-internal-real-time)) ; ensure we can parallelize this
	 (dirpath "/Users/dr/path-data/part2/*.log") ; "/Volumes/dr/path-data/part2/*.log"   "~/path-data/part2/No15_12.log"
	 (stats-output (open (format nil "/Users/dr/MURI/path-data/model-results.~a.~a.txt" (or file "mc") tme)
			     :direction :output :if-exists (if model-active :overwrite :append) 
			     :if-does-not-exist :create))
	 (*temp-file* (format nil "/tmp/out.~a.png" tme))
	 (*temp-file1* (format nil "/tmp/out2.~a.png" tme))
	 (*temp-file-gif* (format nil "/tmp/out.~a.gif" tme))
	 (files  (directory (pathname dirpath)) )) ;; (glob "*.png")))

    (format stats-output "subj cond time max.straight bot.width distance subj.norm.area succ area plen.subj plen.model blue.density always.check.sideways png ~%")

					;    (loop for *robot-width*
    
					;	 (let ((write-png (eq *straight-line-length* 400)))
    (dotimes (rep (or repetitions 1))
      (let ((write-png (and (= rep 1) *write-png*)) (fill-area nil))
	(loop for log-pathname in files do
	     
					;(loop for *blue-density* in '(0 0.1 0.2 0.4 0.6 0.8) do
					; (loop for *straight-line-length* from 60 to 540 by 60 do
	     (let* ((file (file-namestring log-pathname))
		    (file-path (directory-namestring log-pathname))
		    (id (subseq file 0 (- (length file) 4))))

	       (print file)
	       (ppcre::register-groups-bind 
		(subj cond) ("No([0-9]+)_([0-9]+).log" file)
		...
		(let* ((pixel-size nil)
		  ...
		  (let ((itineraries
			 (loop
				...
				(when geo-data  ; file may not exist, when plans from log are empty
				  (setq pixel-size (first geo-data))
				  (format t "pixel size: ~a" pixel-size)
				  (ppcre::do-register-groups (ystr xstr)
				    ...
				    (let ((x (/ (- (read-from-string xstr) (second geo-data)) pixel-size))
				      ...
				      (push (cons x y)
					    waypoints)))
				  (list (cons time (reverse waypoints))))))))

		    ;; for each itinerary
		    
		    ;; visualize path 
		    (loop for (time . waypoints) in itineraries 
					;No15_12_1215807634875-wayp1215527991281oints.png
		       ;;when (equal time "1215029542890")  ; problematic case
		       when (> (length waypoints) 1) ;; ignore pure waypoint instructions
		       do
			 (if (equal time start-at-time)
			     (setq model-active t))
			 (when model-active


			   (let* ((*always-check-left-and-right* (if (eq always-check 'random) (= (random 2) 0) always-check))
				  ;; Basic Monte-Carlo simulation
				  (*blue-density* (or blue-density (random 1.0)))
				  (straight-line-meters (or straight-line (+ 0.5 (random 20.0)))) ; 4.5 seemed to be optimal in old data
				  (robot-width-meters (or robot-width (+ 0.5 (random 1.0))))
				  (*straight-line-length* (/ straight-line-meters pixel-size))
				  (*robot-width* (/ robot-width-meters pixel-size))) ; slightly wider than the normal width
			     

			     (let ((image-file  (convert-to-gif (format nil "~a~a_~a.png" file-path id time))))
			       (when (probe-file image-file)

				 (let* ((stream (skippy::load-data-stream image-file))
					(image (aref (skippy::images stream) 0))
					;; for flip-point
					(*canvas-width* (skippy::width image))
					(*canvas-height* (skippy::height image))
					 ...
					 (visualize-path (mapcar #'flip-point waypoints) nil *yellow*))
				       ...
				       (setq reached-goal 
					     (graphical-solve image-file (flip-point (car waypoints)) (flip-point (car (last waypoints)))
							      (* 3 (waypoints-len waypoints))))  ;; pre-filter longest paths for better speed
				       (unless reached-goal
					...
					; else (write-png)
					 ...
					 (let ((len-wp (waypoints-len waypoints))
					   ...
					   (when (and (> len-wp 0) (> len-vis 0))
					     (format stats-output "~a ~t ~a ~t ~a ~t ~a ~t ~a ~t ~a ~t ~a ~t ~a ~t ~a ~t ~a ~t ~a ~t ~a ~t ~a ~t ~a~%" 
						     subj cond time 
						     (* *straight-line-length* pixel-size) 
						     (* *robot-width* pixel-size)
						     (* (vectors::point-distance (car waypoints) (car (last waypoints))) pixel-size)
					     ...
					     (when write-png
					       ...
					       (mark-point (flip-point  (first waypoints)) *yellow*)
					       (mark-point (flip-point  (car (last waypoints))) *yellow*)
					       (vecto::save-png *temp-file*)
...
; final2:
;(run-over-geo-files .6 .8 1 nil)  ; blue-density robot-width straight-line always-check)

(run-over-geo-files .6 .8 1 nil 1 "final3")
;(run-over-geo-files .6 .8 nil nil 5 "mc")


;; # blue.density .6
;; # bot width .8
;; # max straight 1
;; # always check sideways = OFF
