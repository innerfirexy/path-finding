...
;; 1. (run-over-geo-files)
;; configure paths in that function
;; IEEE-SMC experiments (Lewis data) - logs from Mrcs experiment

;; 2. (make-maze) ;ignore the errors

;; 3. (graphical-solve image-file start end)
;; visual path finding for image file (gif), from start to end point


;; note, the origin point is on the left-bottom corner

;; for p2s2.png
;; try using a smaller *square-width*: 
;; (setq *square-width* 10)
;; (setq *robot-width* 2)
;; (single-image-solve "p2s2.png" (cons 230 80) (cons 220 20)) 
	;; --> ok with *square-width* set to 8
;; start in the room, end ouside the room
;; (single-image-solve "p2s2.png" (cons 205 60) (cons 220 20))
;; (single-image-solve "p2s2.png" (cons 25 105) (cons 220 20))	left top corner -> right bottom corner
;; (single-image-solve "p2s2.png" (cons 10 60) (cons 30 60))  works well
;; (single-image-solve "p2s2.png" (cons 10 60) (cons 65 50))  go through the first door, but go through the second wall
	;;
	;;
;; (single-image-solve "p2s2.png" (cons 10 80) (cons 220 20))
	;; --> not good with *square-width* as 20, error: SKIPPY:PIXEL-REF
	;; --> ok with *square-width* set to 10
	;; --> not good with *square-width* = 10 and *robot-width* = 2
	;; --> ok with *square-width* = 10, *robot-width* = 4
	;; --> not good with *square-width* = 8 or 4

;; larger map 310 x 150
;; (single-image-solve "p2s2_L.png" (cons 13 100) (cons 270 20))
;; (single-image-solve "p2s2_L.png" (cons 290 100) (cons 270 20))
;;

;; new map t3v1.png 598 x 3288
;; test rooms separately
;; (single-image-solve "t3v1.png" (cons 338 3138) (cons 200 2764))  first room
;; (single-image-solve "t3v1.png" (cons 120 1670) (cons 310 1200))  second room
;; (single-image-solve "t3v1.png" (cons 120 470) (cons 120 830))  third room
;; go through doors
;; 

;; 10/5/2014 reduce the running time of single-image-solve and graphical-solve
;; the current binary map is 598x3288, but the era_actr.gif is 330x1919, 
;; thus (211 1900)-->(361 3255), (180 1394)-->(308 2388)
;; (single-image-solve "t3v1.png" (cons 361 3255) (cons 308 2388))
...
;; --debug--
;; transparent-pixel-p calls skippy::pixel-ref, but transparent-pixel-p is only called once in longest-unobstructed-path-segment, 
;; and it restrict (x, y) within the canvase, so it is not likely to be a bug here
;; THUS, must be the problem of sample-path-density, which calls skippy::pixel-ref, and sample-path-density is called only once in move-to-next-target
;; so try changing the code in sample-path-density
;;
;; why the width of the green path would not change? have something to do with sample-path-density?


;; --change log--
;; 9/11/2014
;; use (round) in the (and) condiiton of sample-path-density to fix the skippy:pixel-ref array out-of-bound error


;; for demo.png
;; (single-image-solve "demo.png" (cons 1000 950) (cons 1000 800))
...
(defparameter *straight-line-length* 10) ; original 150

(defparameter *solvable-maze* t)

(defparameter *square-width* 20)  ;; change to smaller values, original 20
...
;; e.g. (ccl::run-program "ls" '() :output t)
;; note, brew install imagemagick
...
(defun mark-point (p &optional col)
  ...
  (vecto::set-line-width 1) ;; change the line width, original 1
...
;  (walk-to (cons 300 300))
  ...
  (let* (
    ...
    (vecto::set-rgba-stroke 0 1 0 1)
    (vecto::set-line-width 0.1) ;; original 2
    (loop for step from 0 to (expt (/ canvas-width *square-width*) *heuristic-game-over-exp*) ;; heuristic ;;why expt?
        ;until (or (equal (point-square end) (point-square *visual-attention-loc*)) ;; the original end condition: reach the same square
		  ;(< (vectors::point-distance end *visual-attention-loc*) *square-width*))
    	until (< (vectors::point-distance end *visual-attention-loc*) 10) ;; more rigid end condition
       do
...
;; the single-iamge-solve function takes a png image as input, and generates a temp file and an output file
(defparameter *input-file* nil)
(defparameter *waypoints-file* nil)
...
(defun single-image-solve (input-file start end)
	...
	(let* ((*input-file* input-file)
		...
		(*canvas-height* (skippy::height image)))
	;; reverse the y-axis
	(setq start-x (car start))
	(setq start-y (- *canvas-height* (cdr start)))
	(setq end-x (car end))
	(setq end-y (- *canvas-height* (cdr end)))
	(setq *image* image)
	(vecto::with-canvas (:width *canvas-width* :height *canvas-height*)
		(vecto::with-graphics-state
			(graphical-solve *input-file* (cons start-x start-y) (cons end-x end-y))
			;;(visualize-path *visual-trace-path* nil *green*)
			(loop for (p1 p2) on *visual-trace-path* do
				(when (and p1 p2)
					...
					(vecto::stroke)))
			(vecto::save-png *waypoints-file*)
		    (compose-images *input-file* *waypoints-file* *output-file*)))))


;; do NOT use this function
(defun graphical-solve-new (image-file start end)
	(declare (optimize (debug 2)))
	(init-cog-model)
	(setq *solution-squares* nil)
	
	(walk-to start)
	(let* ((stream (skippy::load-data-stream (convert-to-gif image-file)))
		(image (aref (skippy::images stream) 0))
		(canvas-width (skippy::width image))
		(canvas-height (skippy::height image))
		(temp-file (concatenate 'string (subseq image-file 0 (search ".png" image-file)) "-temp.png"))
		(output-file (concatenate 'string (subseq image-file 0 (search ".png" image-file)) "-output.png")))
    ...
    (vecto::with-canvas (:width canvas-width :height canvas-height)
    	(vecto::with-graphics-state
		    ...
		    (loop for step from 0 to (expt (/ canvas-width *square-width*) *heuristic-game-over-exp*) ;; heuristic ;;why expt?
				...
				do
				(move-to-next-target image end))
		    ;; save the waypoints to png
		    (visualize-path *visual-trace-path* nil *green*)
		    (vecto::save-png temp-file)
		    (compose-images image-file temp-file output-file)
		    ;; did we reach the goal?
		    (equal (point-square end) (point-square *visual-attention-loc*))))))
...
(defun move-to-next-target (image end)
  (let* ((goal-angle (vectors::point-angle (vectors::p- end *visual-attention-loc* )))
	      (angle (+ (* -2 pi) (* -1 goal-angle)))) ;; because y-axis is reversed, we need to use NEGATIVE goal-angle
	 ...
	 (let ((best-angle nil) (best-length 999999) (best-target) (best-square) (best-debug-points)
	   ...
	   (loop for xd  in '(0) for yd in '(0) ;in (list 0.5 -0.5 0.5 -0.5) for yd in (list -0.5 0.5 0.5 -0.5) 
		...
		(loop for angle from (+ angle (* -2 pi)) to (+ angle (* 2 pi)) by (/ pi 10) do ;; reduce the running time by reducing the step, original pi/20
			 ...
			 (let* ((target-point (longest-unobstructed-path-segment 
					       (vecto::path vecto::*graphics-state*) ;; to get the path from vecto?
					       image
					       (/ *square-width* 2)
					       end 2 t)) ;;last original value 2, add t to start fast mode
			 				;; params of longest-unobstructed-path-segment:
			 				;; (path skippy-image &optional stop-before-wall end-point end-point-radius fast)
				...
				(length-scaled (if *density-sampling* (sample-path-density image target-point end) ;; ? sample-path-density not understood yet
						   (+ (* 1 goal-dist) (* 0 len)))) ;;)
				)
			 ...
			 ;   (mark-point target-point)
			   ...
			   (when (and target-point
				      ...
				      (< length-scaled best-length)) ;; get the shortest langth-scaled, to minimize the scaled-length between target-point and end
...
;;  		      *red*)
	   ...
	   (when best-target
	     ...
	     (walk-to best-target (> (angle-difference best-angle goal-angle) pi) image end))
	   	...
	   	;; best-target is the prediction of the model
	   	;; best-target == target-point
	  ...
	  (let ((point (vectors::p+ start (vectors::p* dir i))))
	    ...
	    (loop for j from -40 to +40 by 4 sum
		 (let* ((point2 (vectors::p+ start (vectors::p* spp j)))
			(col (if (and (< (round (car point2)) canvas-width) (< (round (cdr point2)) canvas-height) (> (round (car point2)) 0) (> (round (cdr point2)) 0))
				 ;;debug
				 (progn ;;(print (list 'debug- (round (car point2)) (round (- canvas-height (cdr point2))) 'j- j 'canvas-height- canvas-height))
...
(defun walk-to (point &optional backtracking skippy-image end)
  ...
  ;(print (format nil "walking to: ~s" point)) ;; do not print
...
;; 2 - white
  ...
  (let ((transp (list 0))
	...
	(not (member (skippy::pixel-ref skippy-image x y)
			  transp))
	(not (or (member (skippy::pixel-ref skippy-image x y)	;; original (and )
			  transp) ;;#xD0D0D0)
      ...
      (loop with iter =
	   ...
	   (when prev-knot
	     ...
	     (let* ((len (vectors::point-distance knot prev-knot))
	       ...
	       (loop for i from 0 to len by (if fast 3 0.3)  ;; original (if fast 3 0.3)
		  for x = (+ startx (* xt i)) 
			...
			(loop for j in (if (and spp *robot-width*)
			   ...
			   (let  (( x (+ x  (* j (car spp))))
			     ...
			     (if  (and (> x 2) (< x (- canvas-width 2))
				  ...
				  (unless
				      (transparent-pixel-p skippy-image x (- canvas-height y) fast)	;; examine if it is wall, if it's wall, return nil
				    ;; mark checkpoint
...
;; attempt to reduce running time
(setq *density-sampling* nil)
